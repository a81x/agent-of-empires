...
- **daemon:** Publish archive and unarchive commits before acknowledging them. Hold lifecycle ownership through teardown, reject concurrent reservations, and preserve the explicit keep-pane policy. Record stopped runtimes so an explicit start works immediately after unarchive.
- **daemon:** Publish complete trash and restore transactions before returning native receipts. Report worktree relocation outcomes to native clients, preserve occupied restore destinations, and leave restored agents stopped.
- **daemon:** Publish native purge commits and typed cleanup outcomes before acknowledging removal. Retain abandoned reservation cleanup through shutdown and preserve a workspace owner when a sibling was restored.
- **daemon:** Refresh explicit runtime resnapshots from committed state instead of waiting for the push loop.
- **daemon:** Prevent overlapping workspace purges from deadlocking while retaining sibling-first cleanup and in-flight prompt exclusion.
- **daemon:** Add generation-bound purge abandonment with daemon-owned runtime cleanup, without starting another worktree or branch removal. Keep abandonment available when a catalogue writer is waiting behind the purge.
- **daemon:** Preserve live state when publishing a batch of profile commits, and reject incomplete publication metadata without partially replacing the mirror.
- **daemon:** Reject incomplete profile reads and invalid global configuration, retaining the last valid rows without rewriting corrupted data.
- **daemon:** Keep runtime snapshots responsive while a store migration delays session commits. Acquire one migration guard before publication for each commit or polling batch.
- **lifecycle:** Preserve the acquired launch generation across hook reloads and leave a superseding owner’s reservation untouched.
- **capture:** Include profile-configured aliases when checking shared sandbox stores. Native capture rejects incomplete peer registries and replaced profiles without quarantining or rewriting their data.
- **sandbox:** Publish migration plans and complete committed profile bundles through the daemon. Preserve historical registries without adding their rows to the profile mirror, and stop copy or retirement work when native authority is revoked.
- **daemon:** Reject unreadable global, profile or repository configuration during native terminal preparation. Keep the selected backend through store migration, separate trusted host hook inputs from repository overrides, and reuse validated sandbox settings.
- **profiles:** Serialize catalogue mutations with daemon lifecycle and store migration, and publish their complete state before native receipts. Publish the default with the catalogue, including external CLI changes, and retain the last valid catalogue during read failures. Renaming or deleting a bootstrap profile updates current and retained rollback launch references.
- **daemon:** Replace a managed private core when exposure is explicitly requested, retaining the previous policy for `serve --rollback`. Preserve secondary passphrase authentication on restart, reject missing credentials before stopping, and prevent ambient passphrase or CityHall settings from changing a replayed policy.
- **sandbox:** Report failed container stops after inconclusive probes while preserving idempotent recovery when the container is absent.
- **projects:** Commit pin and base-branch changes together, preserve concurrent registrations, serialize cross-scope conflict checks, and reflect shared registries in every aliased scope.
- **storage:** Keep project registry writes and canonical session commits bound to opened directories without recreating deleted profiles. Preserve existing permissions on session files.
- **storage:** Reject aliased session and group registries before cross-profile moves can overwrite shared data, including cross-kind aliases within or across profiles.
- **groups:** Revalidate cross-profile subtree moves under both storage locks, rejecting a removed source group or a newly occupied rename destination without rewriting either profile.
- **daemon:** Add explicitly scoped group-subtree rename and profile moves with reflected snapshot receipts. Preserve live terminal identity, check every member before CityHall mutations, and reject duplicated session IDs before acquiring operation locks.
- **daemon:** Create explicit groups and set their collapsed state through the shared native client, publishing empty-group metadata before returning receipts.
- **purge:** Revalidate execution mode at every teardown and commit barrier, retaining changed sessions without overwriting peer status or lifecycle ownership. Clean freshly validated resource paths after destroy hooks instead of deleting a replacement at the old path.
- **daemon:** Retain active purge namespace authority through deferred reservation release, preventing a queued profile mutation from overtaking cleanup during shutdown.
- **cleanup:** Preserve surviving session paths through symlink aliases, missing descendants and parent traversals. Freeze path and branch references before cleanup can remove an alias.
- **worktrees:** Serialize moved-path reconciliation with resource cleanup, reload durable targets, and publish complete native profiles while enforcing read-only and CityHall policy.
- **purge:** Retain filesystem resources when destructive ACP shutdown remains unproven, and stop workspace cleanup before its owner after a sibling cleanup failure. Report committed removals with cleanup failures instead of claiming complete teardown.
- **acp:** Return a pending-teardown error when shutdown waiting expires, preventing project conversion or agent replacement from proceeding on a false successful stop.
- **tmux:** Resolve kill-session and process targets exactly, and propagate connection failures through agent, host, container and tool terminal stops instead of treating failed presence probes as confirmed absence.
- **cleanup:** Stop all indexed auxiliary terminals and tool sessions through a fallible namespace scan. Retain files after failed tmux teardown during purge, trash relocation and build rollback.
- **acp:** Keep teardown pending while any process-group member survives its leader, rather than treating the leader’s exit as proof that shared resources are idle.
- **acp:** Preserve unresolved runner ownership across teardown retries and revoke the stopped client even when its registry cannot be read or parsed. Retry counts and malformed records no longer authorize cleanup without proof.
- **groups:** Add native empty-only and keep-sessions deletion with explicit profile scope, hidden-member checks, complete snapshot receipts and a typed daemon client. Keeping sessions preserves their running terminals.
- **purge:** Capture resource references before destroy hooks and persist them before irreversible row removal so later purges retain files after unproven ACP shutdown, including after daemon reconstruction. Migration 31 initializes the ownership journal; missing or corrupt journals refuse cleanup.
- **purge:** Discover surviving resource references across every durable profile before cleanup, including linked profiles. Unreadable registries and unresolvable owner paths prevent filesystem cleanup; dangling aliases and required symlink traversals remain protected. Serialize reserved cleanup and profile catalogue changes with identity mutations; refuse cleanup when a removed session identity is reused.
- **trash:** Serialize trash, restore and reconciliation worktree moves with resource cleanup across daemon, CLI and TUI callers. Reload the reserved row before relocation instead of following an earlier path snapshot.
- **session:** Serialize project attachment against resource cleanup and revalidate the durable session and target before creating or moving worktrees. Reject stale plans after trash or another lifecycle transition, and refuse conversion when worker ownership cannot be read.
- **acp:** Exclude sessions being created, deleted or held by a fresh lifecycle reservation from reconciler resume and prompt-drain candidates. Recheck eligibility before fresh spawn preparation.
- **purge:** Stop subsequent named-volume, host worktree, branch, scratch and hook cleanup when sandbox container removal fails. Retain named ignore volumes until a successful removal or confirmed container absence.
- **purge:** Capture runner identity before destroy hooks and persist it before irreversible row removal. Both ordinary and irreversible purges refuse host cleanup while that process group survives, even if a hook removes its worker record. Ordinary purge retains the row and releases its reservation on refusal. Migration 32 preserves legacy ownership and records unresolved capture explicitly.
- **daemon:** Include launch text, favorite and dormant timestamps, observed dead-pane state, sandbox display name, and complete workspace checkout metadata in canonical session snapshots. Per-repo diff bases and workspace cleanup preferences no longer need to be inferred by native clients.
- **tui:** Subscribe to canonical runtime snapshots instead of polling the HTTP session list. Bootstrap the local core once at TUI startup, reuse an existing daemon, and retain it on exit. A disconnected subscription does not respawn the daemon.
- **tui:** Send favorite, snooze and unread changes through a bounded native command lane and apply their state only from canonical snapshots. Preserve selection when reflected metadata reorders rows. Automatic unread marks belong to the daemon; dwell and live viewing submit read intents. Reject disconnected, read-only and unhealthy submissions, invalidate queued permissions across revocation, and keep a session pending until its receipt is reflected. Bootstrap no longer prints CLI announcements into the TUI.
- **daemon:** Run status-transition hooks from canonical publications for every profile, without requiring a TUI. Remove dashboard and attached-terminal hook workers; retain local sounds. Keep debounce state per runtime and invalidate pending transitions when their profile ownership changes. Track commands through shutdown and kill remaining process-group members on normal exit, cancellation or timeout.
- **tui:** Read passive terminal and structured status from canonical snapshots, retaining the last state while disconnected. Remove the local status poller and migrate the obsolete `daemon_sidebar` opt-out. The daemon now samples sandbox health; preserve fresh diagnostics, pane liveness and detection state through passive commits and reloads.
- **tui:** Create sessions through the daemon instead of provisioning in-process. The wizard submits the create request, renders the daemon's phase, hook command and output in the creating preview, and cancels through the daemon; the local creation poller, the synchronous build path, the local rollback and the attach-after-create action are gone. The creation's own reservation row stays out of the sidebar while its placeholder stands in for it, and a wizard trust decision now travels as the request's hook approval.
- **tui:** Submit agent Stop through the native command lane without optimistic status writes or local process teardown. Remove the TUI stop worker and duplicate idle reaper; retain daemon-side idle stopping and refuse disconnected submissions.
- **sandbox:** Preserve stores claimed by retained purge owners, including sibling app namespaces. Reclaim refuses missing or malformed ownership journals instead of treating their stores as orphans.
- **tmux:** Exclude dead panes from live PID lookup so teardown and process listing do not treat their stale numeric PIDs as current process owners.
- **terminal:** Revalidate durable session state under title and lifecycle flocks before auxiliary creation or dead-pane recovery. Refuse purging, trashed or missing rows and keep live panes unchanged.
- **tools:** Revalidate configured tool launches under session lifecycle ownership and resolve commands from durable configuration. Keep readiness waits outside launch locks and remove TUI pre-kills before companion recovery.
- **daemon:** Add native configured-tool readiness with strict storage and configuration reads, initial dimensions, policy checks and publication receipts. Tool names are body data, so names such as `..` cannot alter URL routing.
- **terminal:** Create and recover host shells through the native store, commit their terminal metadata before returning a target, and include publication receipts on HTTP ensures. WebSocket recovery now rejects lifecycle reservations even when the pane is already alive.
- **daemon:** Release native launch namespace, submission and instance locks during pre-launch hooks while retaining durable lifecycle ownership. Reacquire exclusion and revalidate the reservation before spawning, so hooks can reenter the API without deadlocking.
- **daemon:** Add native container-terminal targets and receipts with initial dimensions and fresh lifecycle admission. Release API and launch locks around host hooks, retain the launch reservation, and preserve the container's pinned directory and minted environment during dead-pane recovery.
- **terminal:** Publish named canonical agent and auxiliary observations and route TUI auxiliary stops through the native command lane. Stop only the requested host, container or tool pane, preserve the agent and container lifecycle, and refuse unavailable observations. Refresh observations after ensure, start, stop, archive and trash effects before returning receipts; fence polling against intervening commits.
- **tools:** Record full session and tool ownership on the exact created tmux target, including when the daemon inherits a TUI tmux context. Reject unmarked or malformed ownership and exact-name collisions after renames instead of selecting another tool. Use exact tmux existence checks so a primary terminal can be recreated beside an extra terminal.
- **tmux:** Use exact session targets for attach, capture, input, size and VT ownership, and per-session options. A disappeared target no longer redirects an operation to a prefix-matching neighbour. Host, container and tool attachment share the same transport.
- **tui:** Confirm runtime commands only after their receipt is reflected in the applied UI snapshot. Auxiliary handoffs also require the same live tmux name in canonical observations, including renamed tools. Preserve in-flight exclusion across permission revocation and reject revoked receipts even after permission returns.
- **tui:** Prepare auxiliary attachments asynchronously through the native lane and use the daemon-returned target. Cancel stale handoffs after navigation, view, profile, overlay or permission changes. Show canonical runtime availability instead of local daemon PID state, retaining disconnected rows without respawning the daemon.
- **daemon:** Prepare agent panes through the native launch phases with readiness, initial dimensions and publication receipts. Refuse ambiguous pane identities instead of launching another agent. Read-only ensures inspect live panes without relaunching. Failed resume preserves the conversation for explicit retry and exposes a fixed error code without returning authenticated error bodies to native clients.
- **daemon:** Create sessions through the native store with atomic row/group publication, launch reservations, initial terminal dimensions and canonical receipts for fresh requests and idempotent retries. Share typed creation requests and receipt decoding with the native client. Keep admitted creation alive after client disconnect and settle structured launch ownership before subsequent lifecycle operations.
- **session:** Reject invalid creation configuration before provisioning worktrees instead of silently using defaults.
- **daemon:** Refuse profile rename and deletion during native creation without blocking reentrant hooks on other profiles. Retain the reservation until every concurrent creation finishes.
- **daemon:** Accept canonical session IDs as fork sources, resolving provider conversation IDs server-side. Reject missing, busy or incompatible sources and preserve idempotent replay after the parent is removed.
- **daemon:** Publish creation reservations before directory and Git provisioning so reentrant hooks preserve borrowed resources. Roll back failed provisioning and hooks under the matching lifecycle generation and shared-resource guards, including branches created before a failed checkout.
- **session:** Validate resource paths before provisioning, validate every workspace repository before creating directories, and refuse existing targets. Failed validation no longer removes a directory owned by another operation; partial cleanup retains contents it could not safely remove.
- **session:** Preserve pre-existing workspace branches during deletion by recording branch ownership independently from checkout ownership.
- **daemon:** Persist repository hook and MCP approval before directory and Git provisioning; abort creation when approval cannot be saved.
- **daemon:** Accept explicit `trust_hooks: false` to skip untrusted repository hooks and MCP while preserving global/profile hooks and existing approvals. Omitted decisions and plugin creation remain fail-closed.
- **daemon:** Review repository trust through the native API without exposing MCP environment/header values or provisioning resources. Revalidate the captured source and executable configuration before granting approval or creating a session, with a fixed stale-review error for native clients.
- **daemon:** Accept deferred creation cancellation through the native API and SDK. Fence it before runtime launch, roll back owned resources under the creation generation, and preserve admitted work across client disconnection.
- **daemon:** Report live creation phase, hook command and bounded output tail on the runtime stream, limited to the local-owner stream because hook output is the documented secret channel. The canonical snapshot keeps carrying session state alone.
