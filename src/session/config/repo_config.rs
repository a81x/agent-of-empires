...
///
/// Both `run_hook_with_timeout` (captured output) and `run_hooks_streamed`
/// (live progress) produce this value, and each reads
/// [`crate::session::recovery::current_hook_timeout`], so it only appears when
/// a scope is active. Production installs one only inside the startup-recovery
/// cascade (`run_recovery_for_instance`); observing a `HookTimeout` in the
/// error chain therefore still implies the failure happened under a recovery
/// scope.
///
