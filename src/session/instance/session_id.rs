    ...
    pub(super) fn pi_extension_launch(
        ...
        if self.is_sandboxed() {
            ...
            if !container_config.uses_default_container_home()
                || !container_config.path_is_mounted(
                    &bind_dir,
                    std::path::Path::new("/root/.pi"),
                    true,
                )
            {
                return None;
        ...
        if super::launch_command::environment_defines_path(&config.profile.environment)
            || self
                .pending_host_env
                .iter()
