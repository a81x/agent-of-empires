...
`aoe add` names a session from `--title`, else the worktree branch, else a generated name (`-i` prompts like the TUI and wizard do). A structured session still carrying its generated name is auto-renamed from its first turn by a one-shot call to the session's own agent, once the turn finishes, so the title reflects both your prompt and the response. This is `session.smart_rename` (on by default): it renames the title only, never a session you named, and never the worktree directory. `session.smart_rename_agent` points those one-shot calls (rename and conversation summary) at a different agent.
