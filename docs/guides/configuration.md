...
| `sidebar_position` | `"left"` | Place the TUI session list on the `left` or `right`. In Settings, select Global > Session > Sidebar Position, press `Enter` to change sides, and save with `Ctrl+S`. Applies to all profiles and survives restarts. Narrow terminals keep the list above the preview; a collapsed sidebar stays on the chosen side. |
